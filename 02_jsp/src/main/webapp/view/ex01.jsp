	  ...
	  ** 주의 사항 : 선언부, 스크립트릿, 표현식을 겹쳐서(포함해서) 사용할 수 없다.
	  ...
	  1. 지시어 : <%@ page%>, <%@include %>, <%@taglib %>
	  	1) page 지시어 : 보통 해당 문서의 맨 앞에 나온다.
	  				   ...
	  				   톰켓이 해당 page 지시어를 보고, 해당 페이지를 어떻게 해석하고 실행하는지를 판단
	  				   (보통은 그대로 사용한다)
	 	   page 지시어 속성들
	 	   - language : 스크립트 코드에서 사용하는 언어 지정(java)
	 	   - **contentType : 해당 문서의 타입과 해당 문자셋 지정 (캐릭터 인코딩)
	 	   		-- 인코딩(부호화, 암호화) : 정보의 형태를 변환
	 	   		-- 디코딩(복호화)       : 변환된 정보를 원래 형태로 되돌리는 것
	 	   - pageEncoding : 해당 페이지를 utf-8형태로 변환해서 처리하겠다는 뜻
	 	   - session : 세션(저장 공간)을 사용 여부(기본 : true)
	 	   - buffer 버퍼(임시 기억)  : 출력에 사용되는 임시 저장 공간 (기본 : 8kb)
	 	   - autoFlush : 출력 버퍼가 다 차지 않아도 바로 출력 시켜줌 (기본 : TRUE)
	 	   - errorPage : 해당 페이지가 에러를 발생했을 때 처리하는 오류 페이지 지정
	 	   - isErrorPage : 해당 페이지가 오류를 처리할 수 있는 페이지이면 true, 아니면 false (기본: false)
	 	...
	 	3) include 지시어 : HTML의 iframe과 비슷하다. 
	 	   ...
	 	   ** include는 지시어를 사용하는 방법과 includ 액션 태그를 사용하는 방법 두가지다.
 --%>
