...
<%--
	세션과 쿠키
	세션(서버) : 서버와 클라이언트 간의 접속을 유지 시켜주기 위한 객체
			 (http는 요청을 하고 응답을 하면 접속이 끊어진다)
			 클라이언트에서 서비스를 요청하면 서버 측에서 클라이언트에게 서비스를 제공하면서 클라이언트를 구별할 수 있는
			 세션 ID를 부여한다.
	...
	데이터 호출 : session.setAttribute("이름);
    ...
    ** 세션 생명 주기는 제한 시간을 지정하지 않으면 브라우저가 종료되면 세션 데이터도 사라진다.
