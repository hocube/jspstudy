...
<body>
<%-- 
	pageContext 객체는 JSP 페이지에서 1:1 연결된 객체로 JSP 페이지당 하나의 pageContext 객체가 생성된다.
	...
	주요 기능은 다른 기본 내장 객체를 호출하거나, 페이지 흐름을 제어할 때 사용
		- 다른 기본 내장 객체 호출
		  --getRequest(), getResponse(), getOut(), getSession()
		  	...
		  	getServletConfin() => config 객체 구하기
		  	...
		  	** 속성을 이용해서 데이터 저장 및 호출하는 내장 객체들
		  	...
		  	생명 주기 : pageContext < request < Session < application
		  		...
		  		** 속성을 이용해서 저장된 데이터는 EL(표현 언어=${이름})를 사용할 수 있다.
--%>
<%
	//데이터 저장
...
%>
	...
	<%-- pageContext가 우선순위 가장 높으니깐 현재는 홍길동이 나옴.
		하지만 만약 pageContext를 주석처리하면 그 다음으로 우선순위 높은 request의 둘리가 나옴. --%>
	<h1>EL(표현언어)</h1>
	<h2>이름 : ${name}</h2>
