...
<script type="text/javascript">
   ...
   function send_go2(){
      location.href= "ex05_result.jsp?name=둘리&age=24&gender=남&hobby=운동&hobby=독서";
   }
</script>
</head>
<body>
<%--
    ...
    ** 리다이렉트를 실행하면 request, response를 새로만든다.(즉, 파라미터값으 가지고 있지 않다.)
    
    ============================================================================
    웹 페이지 이동
    1. a 태그를 통해서 웹 페이지 이동
       <a href="이동할 주소">텍스트</a>
       // 파라미터값을가지고 a 태그를 통해서 웹 페이지 이동
 ...
 <p><a href="ex05_result.jsp?name=둘리&age=24&gender=남&hobby=운동&hobby=독서">이동하기(파라미터있음)</a></p>
 <hr>
 
 <button onclick="send_go1()">이동하기(파라미터 없음)</button>
 <button onclick="send_go2()">이동하기(파라미터 있음)</button>
 
 <%-- 리다이렉트 --%>
<%--  <% response.sendRedirect("ex02_script.jsp"); %> --%>
 <%-- response.sendRedirect("ex05_result.jsp?name=둘리&age=24&gender=남&hobby=운동&hobby=독서"); --%>
 
  <%-- 포워딩 --%>
  <%-- request.getRequestDispatcher("ex02_script.jsp").forward(request, response); --%>
  <% request.getRequestDispatcher("ex05_result.jsp?name=둘리&age=24&gender=남&hobby=운동&hobby=독서").forward(request, response); %>
</body>
</html>