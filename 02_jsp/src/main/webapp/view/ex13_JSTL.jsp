...
<%-- 
 ...
 <%-- 자바의 String str = "Hell JSTL~~~~~~!!!!"과 같다. --%>
 
 <%-- 변수 출력 : out  <c:out value="출력할 내용 또는 EL표시(변수값 출력)" /> --%>
 <c:out value="str" /> <br> <%-- 이건 그냥 문자 str을 출력하겠다는 뜻 --%>
 <c:out value="${str}" /> <br>
 ${str} <br>
 <hr>
 삭제 전 : ${str} <br>
 ...
 <%--
 	if문 : else가 없다. 조건식이 참 일때만 실행
 	<c:if test="조건식(관계연산, 비교연산 -> EL방식으로 작성 / var="변수">
 		조건식이 참일 때 실행할 내용
 	</c:if>
  --%>
  <c:if test="${4>3}">
  	<h2>결과 : Hello JSTL@@으컁캬 </h2>
  </c:if>
  ...
  <%--
  	...
  	switch문과 비슷하다.
  	<c:choose>
   	<c:when test="조건식(관계연산,비교연산) -> EL방식으로 작성">참일 때 실행</c:when>
   	<c:when test="조건식(관계연산,비교연산) -> EL방식으로 작성">참일 때 실행</c:when>
   	<c:when test="조건식(관계연산,비교연산) -> EL방식으로 작성">참일 때 실행</c:when>
   	<c:when test="조건식(관계연산,비교연산) -> EL방식으로 작성">참일 때 실행</c:when>
   	<c:when test="조건식(관계연산,비교연산) -> EL방식으로 작성">참일 때 실행</c:when>
   ...
   <%--
   	  for문
   		...
   		<c:forEach begin="시작값" end="끝값" step="증가/감소 단계" 
   			var="변수이름" varStatus="변수상태(index값)">
    			 반복 내용 ${변수이름}
    	</c:forEach>
    
    	2. 개선된 for문
    	<c:forEach var="변수이름" items="${배열 또는 컬렉션}" varStatus="변수상태(index값)">
    			반복 내용 ${변수이름}
    	</c:forEach>
    --%>
